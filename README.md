👋 Hola, soy José Gerónimo

🎓 Estudiante de Ingeniería de Sistemas (8.º ciclo – Quinto Superior)
💻 Interesado en Soporte TI, Infraestructura y Gestión de Entornos Microsoft
📍 Lima, Perú

🚀 Perfil Profesional

Tengo experiencia en:

Soporte operativo a usuarios internos
Gestión y validación de información en sistemas corporativos
Registro y control de inventario TI
Elaboración y seguimiento de reportes de incidencias
Uso de Microsoft 365 para gestión y seguimiento de casos

🛠️ Tecnologías y Herramientas
Python (Intermedio)
SQL (Intermedio)
Excel / Office 365
Power BI
Git / GitHub
Windows Server (intermedio)
Configuración básica de redes
Soporte técnico hardware y software

📚 Cursos Relevantes
Cisco Networking Academy – Introduction to Networks
Cisco Networking Academy – Introduction to Cybersecurity
AWS – Machine Learning for NLP

📌 Proyectos Destacados
🔹 Detector de Tendencias Globales
Análisis y procesamiento de datos usando Python, limpieza de datos y visualización para apoyo en toma de decisiones.

🔹 Talento Inclusivo
Plataforma web desarrollada con HTML, CSS y JavaScript enfocada en gestión de información.

<!--
**Jose-Is-gb/Jose-Is-gb** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
